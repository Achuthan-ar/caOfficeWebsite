            ...
            <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 rounded-xl p-5 shadow-sm flex flex-col h-[550px]">
              ...
              <div className="flex-1 overflow-y-auto py-4 space-y-4 text-xs pr-1">
                ...
                {selectedTicket.comments?.map((c, i) => (
                  <div key={i} className={`flex flex-col gap-1 max-w-[85%] ${c.user === user.id ? 'ml-auto items-end' : 'mr-auto items-start'}`}>
              ...
              <div className="pt-4 border-t border-slate-150 dark:border-slate-900 space-y-3.5">
                ...
                {selectedTicket.status === 'Closed' ? (
                  ...
                  <form onSubmit={handleAddComment} className="flex gap-2">
                    <input
                      type="text"
                      placeholder="Type comments, advises, or instructions..."
                      value={commentText}
                      onChange={(e) => setCommentText(e.target.value)}
                      className="flex-1 px-4 py-2.5 border border-slate-200 dark:border-slate-800 rounded-xl bg-transparent focus:outline-none focus:border-indigo-500 transition text-slate-800 dark:text-white text-xs"
                      required
                    />
                    <button
                      type="submit"
